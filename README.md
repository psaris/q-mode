...
C-s` sends the symbol at point, `C-c C-r` sends the selected region
and `C-c C-b` sends the whole buffer.  If prefixed with `C-u C-u`, or
pressing `C-c M-j` `C-c M-f` `C-c M-s` `C-c M-r` respectively, will
also switch point to the active q process buffer for direct
interaction.  Evaluations performed in non-root namespaces are
pre/post-fixed with a command to change directory.
